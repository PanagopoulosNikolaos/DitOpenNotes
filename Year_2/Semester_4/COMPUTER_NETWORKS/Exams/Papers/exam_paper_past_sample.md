...
2. **$d_{\text{queue}}$ (Καθυστέρηση Ουράς / Queuing Delay):** Ο χρόνος αναμονής του πακέτου στην ουρά του μεταγωγέα/δρομολογητή μέχρι να έρθει η σειρά του για μετάδοση. Εξαρτάται από το ρυθμό άφιξης άλλων πακέτων και το μέγεθος του buffer.
...
**Εφαρμογή για AM = 3323:**
- $N = 3323$, τελευταίο ψηφίο $d = 3$
- $R = 3323\text{ KB/s} = 3323 \times 1000 \times 8 = 26.584.000\text{ bps}$
- $D = 3\text{ ms} = 0,003\text{ s}$
- $\text{Max Bits} = 26.584.000\text{ bps} \times 0,003\text{ s} = `79.752\text{ bits}`$

*(Σημείωση: Αν χρησιμοποιηθεί η δυαδική προσέγγιση όπου $1\text{ KB} = 1024\text{ Bytes}$, τότε $R = 3323 \times 1024 \times 8 = 27.222.016\text{ bps}$, οπότε $\text{Max Bits} = 27.222.016 \times 0,003 = `81.666\text{ bits}`$).*
...
- **Κανόνας του Karn:** Για να αποφευχθεί ο διφορούμενος υπολογισμός λόγω επαναμεταδόσεων (retransmission ambiguity), το TCP λαμβάνει δείγματα SampleRTT *μόνο* για τμήματα που μεταδίδονται επιτυχώς με την πρώτη προσπάθεια (χωρίς επανεκπομπή).
- **TCP Timestamps Option (RFC 7323):** Μια σύγχρονη εναλλακτική είναι η ενσωμάτωση χρονικών σημάντρων (timestamps) στην επικεφαλίδα του TCP. Ο αποστολέας θέτει το χρόνο αποστολής και ο παραλήπτης τον επιστρέφει στο ACK, επιτρέποντας τον ακριβή υπολογισμό του RTT ακόμα και σε επαναμεταδόσεις.
...
Λειτουργεί στέλνοντας μια σειρά από IP πακέτα με αυξανόμενο χρόνο ζωής (**TTL - Time To Live**), ξεκινώντας από $TTL = 1$. Κάθε ενδιάμεσος δρομολογητής μειώνει το TTL κατά 1. Όταν το TTL γίνει 0, ο δρομολογητής απορρίπτει το πακέτο και στέλνει πίσω ένα μήνυμα σφάλματος **ICMP Time Exceeded** (Type 11), αποκαλύπτοντας την ταυτότητα (IP διεύθυνση) και το RTT μέχρι τον εκάστοτε ενδιάμεσο κόμβο.

**c.** Υποθέτουμε ότι σας διατίθεται ένας σύνδεσμος για να συνδεθείτε απευθείας με τον server που φιλοξενεί τον δικτυακό τόπο του Τμήματος και ο χρόνος διάδοσης στο σύνδεσμο είναι 5 $ms$ , και ο ρυθμός μετάδοσης είναι 0,125 GB/s. Αν θέλετε να κατεβάσετε ένα πολύ-πολύ μεγάλο αρχείο, ποιος είναι ο μέγιστος αριθμός των bits που μπορούν να σταλούν από τον αποστολέα μέχρι να φτάσει το 1ο bit στη συσκευή σας (bits in flight);
...
- **Βάσει της διατύπωσης της εκφώνησης ($d_{\text{prop}} = 5\text{ ms} = 5 \times 10^{-3}\text{ s}$):**
  $$\text{Bits in flight} = R \times d_{\text{prop}} = 10^9\text{ bps} \times (5 \times 10^{-3}\text{ s}) = `5.000.000\text{ bits}` = `625.000\text{ Bytes}`$$

*(Σημείωση: Αν στην εκτύπωση του θέματος εννοείτο $5\ \mu\text{s} = 5 \times 10^{-6}\text{ s}$ αντί για $5\text{ ms}$, τότε $\text{Bits in flight} = 10^9\text{ bps} \times (5 \times 10^{-6}\text{ s}) = `5000\text{ bits}` = `625\text{ Bytes}`$. Επίσης, με δυαδική σύμβαση $1\text{ GB} = 2^{30}\text{ Bytes}$, για $5\text{ ms}$ προκύπτει $\text{Bits in flight} \approx `5.368.709\text{ bits}`$, ενώ για $5\ \mu\text{s}$ προκύπτει $\approx `5369\text{ bits}`$).*
...
**Εικόνα 4:**
- Η AT&T έχει δύο εξόδους: την $F$ (σύνδεση με $A$) και την $J$ (σύνδεση με $E$).
...
**Προσδιορισμός Μεγέθους Πακέτου με ΑΜ = 3323:**
- **Κύρια προσέγγιση (AM σε Bytes):**  
  Στα δίκτυα, τα μεγέθη πακέτων προσδιορίζονται τυπικά σε Bytes. Με $\text{AM} = 3323$:
  $$\text{Μέγεθος Πακέτου σε Bytes} = \frac{3323}{2} = 1661,5\text{ Bytes}$$
  $$P = 1661,5 \times 8 = 4 \times 3323 = `13.292\text{ bits}`$$
- **Εναλλακτική προσέγγιση (AM απευθείας σε bits):**
  $$P = \frac{3323}{2} = `1661,5\text{ bits}`$$

**Υπολογισμός Επιμέρους Καθυστερήσεων των Ζεύξεων:**
- $d_{\text{prop1}} = \frac{L_1}{u_1} = \frac{10.000\text{ m}}{2,5 \times 10^8\text{ m/s}} = 4 \times 10^{-5}\text{ s} = 0,04\text{ ms}$
- $d_{\text{prop2}} = \frac{L_2}{u_2} = \frac{50.000\text{ m}}{2,5 \times 10^8\text{ m/s}} = 2 \times 10^{-4}\text{ s} = 0,2\text{ ms}$
- Για $P = 13.292\text{ bits}$:
  $$d_{\text{trans1}} = \frac{P}{R_1} = \frac{13292}{12000}\text{ s} = \frac{3323}{3000}\text{ s} \approx 1,107667\text{ s}$$
  $$d_{\text{trans2}} = \frac{P}{R_2} = \frac{13292}{15000}\text{ s} \approx 0,886133\text{ s}$$
  $$d_{\text{trans1}} + d_{\text{trans2}} = 13292 \times \left( \frac{1}{12000} + \frac{1}{15000} \right) = 13292 \times 0,00015 = 1,9938\text{ s}$$

---

**A. Χρόνος αποστολής ενός πακέτου από τον A στον C:**
Λόγω της αρχής Store-and-Forward στον ενδιάμεσο κόμβο $B$:
$$t_{\text{total}} = d_{\text{trans1}} + d_{\text{prop1}} + d_{\text{trans2}} + d_{\text{prop2}}$$
Γενικός μαθηματικός τύπος:
$$t_{\text{total}} = P \times \left( \frac{1}{12000} + \frac{1}{15000} \right) + (4 \times 10^{-5} + 2 \times 10^{-4}) = 1,5 \times 10^{-4} \times P + 0,00024\text{ s}$$

**Αριθμητικός υπολογισμός για AM = 3323 ($P = 13.292\text{ bits}$):**
$$t_{\text{total}} = 1,5 \times 10^{-4} \times 13292 + 0,00024 = 1,9938 + 0,00024 = `1,99404\text{ s}`$$

*(Για την προσέγγιση $P = 1661,5\text{ bits}$: $t_{\text{total}} = 1,5 \times 10^{-4} \times 1661,5 + 0,00024 = `0,249465\text{ s}`$)*.

---

**B. RTT για αποστολή από A σε B και άμεση επιστροφή (χωρίς $d_{\text{proc}}$):**
$$\text{RTT}_{A-B} = 2 \times d_{\text{trans1}} + 2 \times d_{\text{prop1}} = 2 \times \frac{P}{12000} + 2 \times (4 \times 10^{-5})$$
Γενικός μαθηματικός τύπος:
$$\text{RTT}_{A-B} = \frac{P}{6000} + 8 \times 10^{-5}\text{ s}$$

**Αριθμητικός υπολογισμός για AM = 3323 ($P = 13.292\text{ bits}$):**
$$\text{RTT}_{A-B} = \frac{13292}{6000} + 0,00008 = \frac{3323}{1500} + 0,00008 \approx 2,215333 + 0,00008 = `2,215413\text{ s}`$$
$$\text{RTT}_{A-B} \approx `2,2154\text{ s}`$$

*(Για την προσέγγιση $P = 1661,5\text{ bits}$: $\text{RTT}_{A-B} = \frac{1661,5}{6000} + 0,00008 \approx `0,276997\text{ s}`$)*.

---

**C. RTT για αποστολή από A σε C και επιστροφή (με $d_{\text{proc}} = 0,02\text{ ms} = 2 \times 10^{-5}\text{ s}$):**
Η πλήρης διαδρομή είναι $A \rightarrow B \rightarrow C \rightarrow B \rightarrow A$.
Περιλαμβάνει 4 μεταδόσεις, 4 διαδόσεις και 3 ενδιάμεσες επεξεργασίες κόμβων (στον $B$ κατά τη μετάβαση, στον $C$ πριν την αναστροφή, και στον $B$ κατά την επιστροφή):
$$\text{RTT}_{A-C} = 2 \times t_{\text{total}} + 3 \times d_{\text{proc}}$$
Γενικός μαθηματικός τύπος:
$$\text{RTT}_{A-C} = 2 \times (1,5 \times 10^{-4} \times P + 0,00024) + 3 \times (2 \times 10^{-5})$$
$$\text{RTT}_{A-C} = 3 \times 10^{-4} \times P + 0,00048 + 0,00006 = 3 \times 10^{-4} \times P + 0,00054\text{ s}$$

**Αριθμητικός υπολογισμός για AM = 3323 ($P = 13.292\text{ bits}$):**
$$\text{RTT}_{A-C} = 3 \times 10^{-4} \times 13292 + 0,00054 = 3,9876 + 0,00054 = `3,98814\text{ s}`$$

*(Αν συνυπολογιστεί και η επεξεργασία στον κόμβο $A$ κατά την τελική λήψη [4 κόμβοι]: $\text{RTT}_{A-C} = 3,9876 + 0,00056 = `3,98816\text{ s}`$).*  
*(Για την προσέγγιση $P = 1661,5\text{ bits}$: $\text{RTT}_{A-C} = 3 \times 10^{-4} \times 1661,5 + 0,00054 = `0,49899\text{ s}`$)*.
