...
### Λύση Θέματος 3
Οι εντολές παραμετροποίησης OSPF στο Cisco IOS για τον δρομολογητή $R$ που συνδέεται στα 4 υποδίκτυα είναι οι εξής:
```
R>en
...
- **Διαδρομή:** $X \rightarrow E \rightarrow F \rightarrow A \rightarrow B \rightarrow C \rightarrow D \rightarrow Y$.
...
- Υπάρχει ισοπαλία στο hot-potato routing. Η επιλογή θα γίνει με βάση δευτερεύοντες κανόνες (π.χ. χαμηλότερο Router ID του γειτονικού BGP peer). Αν επιλεγεί η έξοδος $J$, η διαδρομή είναι σημαντικά συντομότερη συνολικά ($X \rightarrow H \rightarrow I \rightarrow J \rightarrow E \rightarrow Y$, 5 hops), ενώ αν επιλεγεί η $F$ η διαδρομή είναι 9 hops.
...
**Α. Χρόνος αποστολής $A \rightarrow C$ (Store and Forward στο B):**
...
**C. RTT μεταξύ A και C με καθυστέρηση επεξεργασίας $d_{\text{proc}} = 0,02\text{ ms} = 2 \times 10^{-5}\text{ s}$:**
...
### Λύση 2ης Άσκησης
**Α) Πίνακας Dijkstra από τον κόμβο E:**
...
**Β) Ελάχιστο μέγεθος πλαισίου CSMA/CD:**
Για την ανίχνευση σύγκρουσης, η καθυστέρηση μετάδοσης πρέπει να είναι τουλάχιστον διπλάσια από την καθυστέρηση διάδοσης:
$$t_{\text{trans}} \ge 2 \times t_{\text{prop}} \Rightarrow \frac{L_{\text{min}}}{R} \ge 2 \times t_{\text{prop}} \Rightarrow L_{\text{min}} \ge 2 \times t_{\text{prop}} \times R$$
- $R = 15\text{ Mbps} = 15 \times 10^6\text{ bps}$
- $t_{\text{prop}} = 25,6\ \mu\text{s} = 25,6 \times 10^{-6}\text{ s}$

$$L_{\text{min}} = 2 \times (25,6 \times 10^{-6}\text{ s}) \times (15 \times 10^6\text{ bps}) = 2 \times 25,6 \times 15 = 768\text{ bits}$$
Σε Bytes: $\frac{768}{8} = 96\text{ Bytes}$.

---

### Λύση 3ης Άσκησης (Bonus)
...
- **$P_1$ (θέσεις 1, 3, 5, 7, 9, 11):**
  $P_1 \oplus 1 \oplus 0 \oplus 0 \oplus 1 \oplus 1 \Rightarrow P_1 \oplus 3$.
  Για να έχουμε περιττό αριθμό άσων, πρέπει $P_1 = 0$ (έτσι έχουμε 3 άσους).
- **$P_2$ (θέσεις 2, 3, 6, 7, 10, 11):**
  $P_2 \oplus 1 \oplus 0 \oplus 0 \oplus 0 \oplus 1 \Rightarrow P_2 \oplus 2$.
  Για να έχουμε περιττό αριθμό άσων, πρέπει $P_2 = 1$ (έτσι έχουμε 3 άσους).
- **$P_4$ (θέσεις 4, 5, 6, 7, 12):**
  $P_4 \oplus 0 \oplus 0 \oplus 0 \oplus 1 \Rightarrow P_4 \oplus 1$.
  Για να έχουμε περιττό αριθμό άσων, πρέπει $P_4 = 0$ (έτσι έχουμε 1 άσο).
- **$P_8$ (θέσεις 8, 9, 10, 11, 12):**
  $P_8 \oplus 1 \oplus 0 \oplus 1 \oplus 1 \Rightarrow P_8 \oplus 3$.
