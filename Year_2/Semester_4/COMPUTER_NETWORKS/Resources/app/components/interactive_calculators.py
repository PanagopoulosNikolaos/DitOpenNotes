    ...
    with ui.column().classes("w-full glass-panel p-6 rounded-2xl gap-5 latex-target"):
        ...
        def updateDelayResults() -> None:
            ...
            try:
                ...
                with results_container:
                    with ui.grid().classes("grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 w-full text-xs"):
                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(224,107,58,0.3)] gap-1"):
                            ui.label("d_trans (1 link)").classes("text-[#b5b0a4]")
                        ...
                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(16,185,129,0.3)] gap-1"):
                            ui.label("d_prop (1 link)").classes("text-[#b5b0a4]")
                        ...
                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(79,142,201,0.3)] gap-1"):
                            ui.label("Total d_proc").classes("text-[#b5b0a4]")
                        ...
                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(245,158,11,0.5)] gap-1"):
                            ui.label("Total Time T_total").classes("text-[#b5b0a4]")
                    ...
                    with ui.column().classes("w-full p-4 rounded-xl bg-[#141413] border border-[rgba(255,255,255,0.06)] font-mono text-xs text-[#fed7aa] gap-1"):
                        ui.label("Step-by-Step Calculation Breakdown:").classes("text-stone-300 font-bold")
                ...
                with results_container:
                    ui.label(f"Calculation error: {str(e)}").classes("text-red-400 text-xs")
...
def calculateCrcDivision(data_bits: str, generator_bits: str) -> Tuple[str, List[str], str]:
    ...
    if not clean_d or not clean_g or len(clean_g) < 2:
        return "0", ["Invalid input data"], clean_d
    ...
    trace.append(f"Appending {k} zeros: {''.join(dividend)}")

    for i in range(len(clean_d)):
        if dividend[i] == '1':
            step_str = f"Step {i+1}: XOR with generator at bit {i}"
    ...
    trace.append(f"Calculated Remainder (FCS / CRC): {remainder}")
    trace.append(f"Transmitted Frame T = D + R: {transmitted}")
...
def renderCrcCalculator() -> None:
    ...
    with ui.column().classes("w-full glass-panel p-6 rounded-2xl gap-5 latex-target"):
        with ui.row().classes("items-center gap-2 border-b border-[rgba(255,255,255,0.08)] pb-3"):
            ...
            ui.html('<h3 class="text-lg font-bold m-0 text-[#f4f1ea]">Interactive CRC Calculator (Modulo-2 XOR Division)</h3>')

        ui.label(
            "Enter binary data (D) and generator polynomial (G) to observe "
            "step-by-step Modulo-2 binary division and frame construction."
        ).classes("text-xs text-[#b5b0a4]")

        with ui.grid().classes("grid-cols-1 md:grid-cols-2 gap-4 w-full text-xs"):
            with ui.column().classes("gap-1"):
                ui.label("Data D (Binary String)").classes("font-semibold text-[#fed7aa]")
                data_input = ui.input(value="11010011101100").props("outlined dense dark").classes("w-full font-mono")

            with ui.column().classes("gap-1"):
                ui.label("Generator G (Binary String, e.g. 10011 for x^4 + x + 1)").classes("font-semibold text-[#fed7aa]")
            ...
            with crc_results:
                with ui.grid().classes("grid-cols-1 md:grid-cols-2 gap-3 w-full text-xs"):
                    with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(245,158,11,0.3)] gap-1"):
                        ui.label("CRC Remainder / FCS (k bits)").classes("text-[#b5b0a4]")
                        ui.label(rem).classes("font-mono text-base font-bold text-amber-400")

                    with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(16,185,129,0.3)] gap-1"):
                        ui.label("Final Transmitted Frame T").classes("text-[#b5b0a4]")
                        ui.label(tx_frame).classes("font-mono text-base font-bold text-emerald-400")

                with ui.column().classes("w-full p-4 rounded-xl bg-[#141413] border border-[rgba(255,255,255,0.06)] font-mono text-xs text-[#fed7aa] gap-1"):
                    ui.label("Modulo-2 Binary Division Execution Trace:").classes("text-stone-300 font-bold")
...
def renderSubnetCalculator() -> None:
    ...
    with ui.column().classes("w-full glass-panel p-6 rounded-2xl gap-5 latex-target"):
        with ui.row().classes("items-center gap-2 border-b border-[rgba(255,255,255,0.08)] pb-3"):
            ...
            ui.html('<h3 class="text-lg font-bold m-0 text-[#f4f1ea]">Interactive Subnetting & LPM Calculator</h3>')

        ui.label(
            "Analyze any IPv4 address and CIDR prefix to immediately determine "
            "the network address, broadcast address, host range, and number of usable hosts."
        ).classes("text-xs text-[#b5b0a4]")

        with ui.grid().classes("grid-cols-1 md:grid-cols-2 gap-4 w-full text-xs"):
            with ui.column().classes("gap-1"):
                ui.label("IPv4 Address (e.g. 192.168.5.130)").classes("font-semibold text-[#fed7aa]")
                ip_input = ui.input(value="192.168.5.130").props("outlined dense dark").classes("w-full font-mono")

            with ui.column().classes("gap-1"):
                ui.label("CIDR Mask / Prefix (0-32)").classes("font-semibold text-[#fed7aa]")
        ...
        def updateSubnet() -> None:
            ...
            try:
                ...
                with subnet_results:
                    with ui.grid().classes("grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 w-full text-xs"):
                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(79,142,201,0.3)] gap-1"):
                            ui.label("Network Address").classes("text-[#b5b0a4]")
                            ui.label(net_ip).classes("font-mono text-base font-bold text-blue-400")

                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(245,158,11,0.3)] gap-1"):
                            ui.label("Subnet Mask").classes("text-[#b5b0a4]")
                        ...
                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(239,68,68,0.3)] gap-1"):
                            ui.label("Broadcast Address").classes("text-[#b5b0a4]")
                            ui.label(bcast_ip).classes("font-mono text-base font-bold text-red-400")

                        with ui.column().classes("p-3 rounded-xl bg-[#201f1d] border border-[rgba(16,185,129,0.3)] gap-1"):
                            ui.label("Usable Hosts").classes("text-[#b5b0a4]")
                            ui.label(f"{host_count:,}").classes("font-mono text-base font-bold text-emerald-400")

                    with ui.column().classes("w-full p-4 rounded-xl bg-[#141413] border border-[rgba(255,255,255,0.06)] font-mono text-xs text-[#fed7aa] gap-1"):
                        ui.label(f"Usable Host IP Range: {first_host} — {last_host}")
                        ui.label(f"Binary Mask: {''.join(f'{((mask_int >> (24-8*i)) & 0xFF):08b}.' for i in range(4))[:-1]}")

            except Exception as e:
                with subnet_results:
                    ui.label(f"IP analysis error: {str(e)}").classes("text-red-400 text-xs")
