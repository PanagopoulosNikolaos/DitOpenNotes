...
def renderQuestionBlock(q: ExamQuestion) -> None:
    ...
    if "Calculat" in q.question_type or "Exercise" in q.title or "Problem" in q.title or q.calculation_steps:
    ...
    with ui.expansion(
        text=f"Question {q.question_number}: {q.title}",
        icon=type_icon,
    ).classes(
        ...
        with ui.column().classes("w-full gap-4 px-3 pb-3 pt-1 latex-target"):
            ...
            with ui.row().classes("items-center gap-2 flex-wrap"):
                ...
                if q.related_theory_topic:
                    ui.label(f"Theory: {q.related_theory_topic}").classes(
            ...
            if q.options:
                ...
                def handleOptionClick(opt_letter: str, is_corr: bool, exp: str) -> None:
                    ...
                    with feedback_container:
                        ...
                        status_text = "Correct Choice" if is_corr else "Incorrect Choice"
                        ...
                        with ui.column().classes(f"w-full p-4 rounded-xl border {text_color} gap-1.5").style(
                            ...
                            with ui.row().classes("items-center gap-2 font-bold text-xs"):
                                ...
                                ui.label(f"Option {opt_letter}: {status_text}")
            ...
            if q.calculation_steps:
                with ui.column().classes("w-full mt-2 gap-0 border-t border-[rgba(255,255,255,0.06)] pt-4"):
                    with ui.row().classes("items-center gap-2 mb-3"):
                        ...
                        ui.label("Step-by-Step Analytical Solution").classes("text-sm font-bold text-[#fed7aa]")
                            ...
                            with ui.column().classes(
                                ...
                                if step.formula:
                                    with ui.column().classes("w-full formula-box gap-0.5"):
                                        ui.label("Formula:").classes("text-[0.65rem] font-bold text-[#78756d] uppercase tracking-wider")
                                        ui.markdown(f"$$${step.formula}$$$").classes("text-[#fed7aa]")

                                # Substitution row
                                if step.substitution:
                                    with ui.column().classes("w-full formula-box gap-0.5"):
                                        ui.label("Substitution:").classes("text-[0.65rem] font-bold text-[#78756d] uppercase tracking-wider")
                                ...
                                if step.result:
                                    with ui.row().classes(
                                        ...
                                        ui.label("Result:").classes("text-xs font-bold text-emerald-400")
            ...
            if q.detailed_justification:
                with ui.column().classes("w-full p-4 rounded-xl bg-[#141413] border border-[rgba(224,107,58,0.2)] gap-2 text-xs"):
                    with ui.row().classes("items-center gap-2 text-[#e06b3a] font-bold"):
                        ...
                        ui.label("Justification & Theoretical Background")
    ...
    with ui.column().classes("w-full glass-panel p-6 rounded-2xl gap-6"):
        with ui.row().classes("items-center gap-2 border-b border-[rgba(255,255,255,0.08)] pb-3"):
            ...
            with ui.column().classes("gap-0"):
                ui.html('<h2 class="text-xl font-bold gradient-title m-0">Detailed Question & Exercise Solutions</h2>')
                ui.label(
                    f"Includes {len(scenario.questions)} questions with full justification, mathematical derivations, and LaTeX formulas. "
        ...
        if part_a:
            with ui.column().classes("w-full gap-3"):
                with ui.row().classes("items-center gap-2.5 py-2 px-4 rounded-xl bg-[rgba(224,107,58,0.1)] border border-[rgba(224,107,58,0.25)]"):
                    ...
                    ui.html('<h3 class="text-sm font-bold text-[#fed7aa] m-0">Part A: Theory & Multiple-Choice Questions</h3>')
                for q in part_a:
        ...
        if part_a and part_b:
            with ui.row().classes("w-full items-center gap-3 my-2"):
                ...
                ui.html(
                    '<span class="text-xs text-[#78756d] font-mono uppercase tracking-widest px-2">Part B</span>',
                    tag="div",
                )
                ui.html('<div class="flex-1 h-px bg-[rgba(255,255,255,0.07)]"></div>', tag="div")
        ...
        if part_b:
            with ui.column().classes("w-full gap-3"):
                with ui.row().classes("items-center gap-2.5 py-2 px-4 rounded-xl bg-[rgba(79,142,201,0.1)] border border-[rgba(79,142,201,0.25)]"):
                    ...
                    ui.html('<h3 class="text-sm font-bold text-blue-300 m-0">Part B: Exercises & Analytical Solutions</h3>')
                for q in part_b:
                    renderQuestionBlock(q)
