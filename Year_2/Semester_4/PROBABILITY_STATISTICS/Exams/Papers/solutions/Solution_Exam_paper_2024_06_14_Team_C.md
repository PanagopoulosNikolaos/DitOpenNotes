...
**PROBLEM 2:** In a survey conducted in a region regarding readership of various Sunday newspapers, 25% stated they read "Ta Nea", 35% "To Vima", and 5% stated they read both newspapers. We select a person at random. Calculate the probability that:
...
**PROBLEM 3:** The time required for bank customers to be served at the teller follows the Normal distribution with mean $\mu=10$ min and standard deviation $\sigma=2$ min. What is the probability that a customer waits:
i) more than 7 min
ii) between 9 and 13 min
iii) what command should we give in R to calculate the probability of question ii)?
...
- Symmetry of the normal: $P(Z > -a) = P(Z \le a)$.
...
**Note:** The data are ungrouped (discrete salary values).
...
- $\frac{N}{4} = 45$. We have $F_1 = 22 < 45 \le 57 = F_2$, so $Q_1$ lies in the category with salary 1300 €.

$$Q_1 = \boxed{1300 \text{ €}}$$

- Since the data are discrete, $Q_1$ is the value corresponding to the class containing the 25th percentile.

**B. Percentage with salary $\le 1300$ €**

$$\text{Percentage} = \frac{F_2}{n} = \frac{57}{180} \approx 0.3167 = \boxed{31.67\%}$$

**C. Allowance cutoff (lowest 25% → $Q_1$)**

$$\text{Cutoff} = Q_1 = \boxed{1300 \text{ €}}$$

- The 25% with the lowest salary correspond to the 1st quartile.

**D. R commands for the mode**

```r
salaries <- c(...)        # data input
# R does not have mode so we use the following:
table(salaries)           # show frequencies
names(which.max(table(salaries)))  # mode
```
