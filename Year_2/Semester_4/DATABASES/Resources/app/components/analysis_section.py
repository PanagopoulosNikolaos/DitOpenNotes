...
def renderAnalysisSection(scenario: Scenario) -> None:
    ...
    with ui.column().classes("w-full gap-6"):
        with ui.row().classes("items-center gap-3 no-print"):
            ...
            ui.html('<h2 class="text-xl md:text-2xl font-bold text-[var(--text-1)] m-0">Αναλυτική Αιτιολόγηση Αναγνώρισης Στοιχείων</h2>')
        ...
        with ui.column().classes("w-full glass-panel gap-4 no-print"):
            with ui.row().classes("items-center gap-2 border-b border-[var(--border)] pb-3 w-full"):
                ...
                ui.html('<h3 class="text-lg font-bold text-blue-600 dark:text-[#93c5fd] m-0">1. Οντότητες & Είδος</h3>')
            ...
            with ui.grid().classes("grid-cols-1 md:grid-cols-2 gap-4 w-full"):
                for idx, ent in enumerate(scenario.entities, start=1):
                    ...
                    with ui.column().classes(f"p-4 rounded-xl border {border_color} gap-2 shadow-sm"):
                        ...
                        ui.html(f'<p class="text-xs text-[var(--text-2)] leading-relaxed m-0"><strong>Αιτιολογία:</strong> {ent.justification}</p>')
        ...
        with ui.column().classes("w-full glass-panel gap-4 print-section print-attributes"):
            with ui.row().classes("items-center gap-2 border-b border-[var(--border)] pb-3 w-full"):
                ...
                ui.html('<h3 class="text-lg font-bold text-emerald-600 dark:text-[#86efac] m-0">2. Γνωρίσματα & Είδος</h3>')
                ...
                if scenario.relationship_attributes:
                    with ui.column().classes("p-4 rounded-xl bg-[var(--card-bg-rel-attr)] border border-[var(--card-border-rel-attr)] gap-2 w-full attr-card-rel print-avoid-break shadow-sm"):
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.label("Γνωρίσματα Σχέσεων (Relationship Attributes):").classes("font-bold text-rose-700 dark:text-[#fda4af] text-sm")
        ...
        with ui.column().classes("w-full glass-panel gap-4 print-section print-keys"):
            with ui.row().classes("items-center gap-2 border-b border-[var(--border)] pb-3 w-full"):
                ...
                ui.html('<h3 class="text-lg font-bold text-amber-600 dark:text-[#fde68a] m-0">3. Ανάλυση Κλειδιών & Τελική Επιλογή Πρωτεύοντος Κλειδιού (PK)</h3>')
        ...
        with ui.column().classes("w-full glass-panel gap-4 print-section print-relationships"):
            with ui.row().classes("items-center gap-2 border-b border-[var(--border)] pb-3 w-full"):
                ...
                ui.html('<h3 class="text-lg font-bold text-rose-600 dark:text-[#fda4af] m-0">4. Σχέσεις, Λόγοι Πληθικότητας & Αιτιολογήσεις</h3>')
            ...
            with ui.column().classes("space-y-4 w-full rel-card-container"):
                for rel in scenario.relationships:
                    rel_bg = (
                        ...
                        if "Ταυτοποιούσα" in rel.relationship_type
                        else "bg-[var(--card-bg-rel)] border-rose-300 dark:border-rose-500/30"
                    )
                    badge_color = "text-purple-700 dark:text-purple-300" if "Ταυτοποιούσα" in rel.relationship_type else "text-rose-700 dark:text-rose-300"
                    with ui.column().classes(f"p-4 rounded-xl border {rel_bg} gap-2 w-full rel-card print-avoid-break shadow-sm"):
                        ...
                        with ui.grid().classes("grid-cols-1 md:grid-cols-2 gap-2 text-xs text-[var(--text-2)]"):
                            ui.html(f'<div><strong>Λόγος Πληθικότητας:</strong> <span class="text-orange-600 dark:text-[#fdba74] font-bold">{rel.cardinality}</span></div>')
                            ui.html(f'<div><strong>Συμμετοχή:</strong> {rel.participation}</div>')

                        if rel.attributes:
                            attr_list_str = ", ".join([f"<code>{a}</code>" for a in rel.attributes])
                            ui.html(f'<div class="text-xs text-[var(--text-2)]"><strong>Γνώρισμα Σχέσης:</strong> {attr_list_str}</div>')

                        ui.html(f'<p class="text-xs text-[var(--text-2)] leading-relaxed m-0"><strong>Αιτιολογία:</strong> {rel.justification}</p>')
        ...
        with ui.column().classes("w-full p-5 rounded-xl bg-[var(--card-bg-subtle)] border border-[var(--border)] gap-3 no-print shadow-sm"):
            with ui.row().classes("items-center gap-2"):
                ...
                ui.label("Παραδοχές Σχεδιασμού (Design Assumptions)").classes("font-bold text-[var(--text-1)] text-sm")
