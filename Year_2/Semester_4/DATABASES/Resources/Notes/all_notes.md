...
6. [Περιορισμοί Συμμετοχής (Participation Constraints)](#περιορισμοί-συμμετοχής-participation-constraints)
   - [Ολική και Μερική Συμμετοχή](#ολική-και-μερική-συμμετοχή)
   - [Συμβολισμός στο ERD](#συμβολισμός-στο-erd)
   - [Επίδραση στο Σχεσιακό Μοντέλο](#επίδραση-στο-σχεσιακό-μοντέλο)
7. [Γνωρίσματα Συσχέτισης (Relationship Attributes)](#γνωρίσματα-συσχέτισης-relationship-attributes)
8. [Πολλαπλές Συσχετίσεις μεταξύ ίδιων Οντοτήτων](#πολλαπλές-συσχετίσεις-μεταξύ-ίδιων-οντοτήτων)
9. [Εξειδίκευση / Γενίκευση (ISA)](#εξειδίκευση--γενίκευση-isa)
10. [Πίνακας Βασικών Εννοιών](#πίνακας-βασικών-εννοιών)
11. [Βασικά Συμπεράσματα](#βασικά-συμπεράσματα)
...
Οι **Περιορισμοί Συμμετοχής (Participation Constraints)** ορίζουν αν **κάθε** στιγμιότυπο μιας οντότητας **πρέπει υποχρεωτικά** να συμμετέχει σε μια συσχέτιση, ή αν η συμμετοχή του είναι προαιρετική. Μαζί με τους περιορισμούς πληθικότητας, περιγράφουν πλήρως τη σύνδεση μεταξύ δύο οντοτήτων.

### Ολική και Μερική Συμμετοχή
*Total and Partial Participation*

- **Ολική Συμμετοχή (Total Participation)**: Κάθε στιγμιότυπο της οντότητας **πρέπει** να συμμετέχει τουλάχιστον μία φορά στη συσχέτιση. Αντιστοιχεί σε **υποχρεωτική (mandatory)** συμμετοχή.
- **Μερική Συμμετοχή (Partial Participation)**: Κάποια στιγμιότυπα της οντότητας **μπορεί να μη συμμετέχουν** στη συσχέτιση. Αντιστοιχεί σε **προαιρετική (optional)** συμμετοχή.

**Παράδειγμα**: Στη συσχέτιση `ΤΜΗΜΑ — Απασχολεί — ΥΠΑΛΛΗΛΟΣ`, κάθε υπάλληλος ανήκει υποχρεωτικά σε ένα τμήμα (ολική συμμετοχή του `ΥΠΑΛΛΗΛΟΥ`), αλλά μπορεί να υπάρχει τμήμα χωρίς υπαλλήλους (μερική συμμετοχή του `ΤΜΗΜΑΤΟΣ`).

### Συμβολισμός στο ERD
*ERD Notation*

- **Μονή γραμμή** μεταξύ οντότητας και συσχέτισης: **Μερική συμμετοχή**.
- **Διπλή γραμμή** μεταξύ οντότητας και συσχέτισης: **Ολική συμμετοχή**.

```text
   Ολική Συμμετοχή ΥΠΑΛΛΗΛΟΥ (διπλή γραμμή):
...
**Exam Note:** Η ολική συμμετοχή συχνά συνδυάζεται με τη διπλή γραμμή, ενώ η πληθικότητα αναγράφεται ως αριθμός (`1`, `N`, `M`) δίπλα στη γραμμή. Μην συγχέετε τη διπλή γραμμή (συμμετοχή) με το διπλό ορθογώνιο (ασθενής οντότητα).

### Επίδραση στο Σχεσιακό Μοντέλο
*Effect on the Relational Model*

Η συμμετοχή καθορίζει αν το ξένο κλειδί επιτρέπει `NULL`:

- **Ολική συμμετοχή** της οντότητας που φέρει το ξένο κλειδί ⇒ το FK ορίζεται **`NOT NULL`**.
- **Μερική συμμετοχή** ⇒ το FK **επιτρέπει `NULL`**.

```sql
-- Ο ΥΠΑΛΛΗΛΟΣ έχει ολική συμμετοχή, άρα το dept_id είναι NOT NULL
CREATE TABLE employees (
    afm     VARCHAR(9)  PRIMARY KEY,
    name    VARCHAR(50) NOT NULL,
    dept_id INT         NOT NULL,          -- Ολική συμμετοχή => NOT NULL
    FOREIGN KEY (dept_id) REFERENCES departments(dept_id)
);

-- Αν η συμμετοχή ήταν μερική, το dept_id θα επέτρεπε NULL:
-- dept_id INT NULL
```

**Key Distinction:** Η **πληθικότητα** απαντά "πόσα" στιγμιότυπα συνδέονται (`1`, `N`, `M`), ενώ η **συμμετοχή** απαντά "αν είναι υποχρεωτικό" να συνδέεται (ολική/μερική). Και τα δύο μαζί περιγράφουν πλήρως τον περιορισμό της συσχέτισης.
...
Τα **Γνωρίσματα Συσχέτισης (Relationship Attributes)** είναι γνωρίσματα που **δεν ανήκουν σε καμία από τις εμπλεκόμενες οντότητες**, αλλά στη **συσχέτιση καθεαυτή**. Περιγράφουν μια ιδιότητα που έχει νόημα μόνο όταν συμμετέχουν μαζί όλες οι συνδεδεμένες οντότητες.

- **Πότε εμφανίζονται**: Κυρίως σε συσχετίσεις **N:M** και, σπανιότερα, σε **1:N**.
- **Παραδείγματα**:
  - `Βαθμολογία` και `Ημ_Εγγραφής` στη συσχέτιση `ΦΟΙΤΗΤΗΣ — Εγγράφεται — ΜΑΘΗΜΑ`.
  - `Ώρες_Εβδομαδιαίως` στη συσχέτιση `ΥΠΑΛΛΗΛΟΣ — Εργάζεται — ΕΡΓΟ`.
  - `Ρόλος` στη συσχέτιση `ΗΘΟΠΟΙΟΣ — Συμμετέχει — ΤΑΙΝΙΑ`.

```text
   +-----------+    N          M    +-----------+
   | ΗΘΟΠΟΙΟΣ |---< Συμμετέχει >---|  ΤΑΙΝΙΑ  |
   +-----------+                   +-----------+
                                     |
                                  ( Ρόλος )   <-- Γνώρισμα Συσχέτισης
```

**Πού καταλήγουν στο σχεσιακό μοντέλο**:

- Σε **N:M** συσχέτιση, τα γνωρίσματα συσχέτισης μεταφέρονται στον **ενδιάμεσο πίνακα (junction table)**, μαζί με τα ξένα κλειδιά.
- Σε **1:N** συσχέτιση, μεταφέρονται στον πίνακα της **"πολλαπλής" πλευράς (Ν)**, μαζί με το ξένο κλειδί.

```sql
-- Γνώρισμα συσχέτισης σε ενδιάμεσο πίνακα N:M
CREATE TABLE participations (
    actor_id   INT  NOT NULL,
    movie_id   INT  NOT NULL,
    role_name  VARCHAR(100) NOT NULL,       -- Γνώρισμα συσχέτισης
...
**Exam Note:** Όταν ζητείται η αναγνώριση γνωρισμάτων συσχέτισης, το κριτήριο είναι: **η τιμή εξαρτάται από τον συνδυασμό** των οντοτήτων, όχι από μία μόνο οντότητα. Ο βαθμός ενός φοιτητή δεν ανήκει ούτε μόνο στον φοιτητή ούτε μόνο στο μάθημα — ανήκει στην εγγραφή τους.

---

## Πολλαπλές Συσχετίσεις μεταξύ ίδιων Οντοτήτων
*Multiple Relationships Between the Same Entity Pair*

Είναι δυνατόν **δύο διαφορετικές συσχετίσεις** να συνδέουν το **ίδιο ζεύγος οντοτήτων**, όταν κάθε σύνδεση έχει διαφορετική σημασία. Οι δύο συσχετίσεις διακρίνονται με **ετικέτες ρόλων (role labels)**.

**Παραδείγματα**:

- `ΑΕΡΟΔΡΟΜΙΟ — Αναχώρηση — ΠΤΗΣΗ` και `ΑΕΡΟΔΡΟΜΙΟ — Άφιξη — ΠΤΗΣΗ` (αεροδρόμιο αναχώρησης / άφιξης).
- `ΟΜΑΔΑ — Εντός_Έδρας — ΑΓΩΝΑΣ` και `ΟΜΑΔΑ — Εκτός_Έδρας — ΑΓΩΝΑΣ` (γηπεδούχος / φιλοξενούμενη ομάδα).
...
```text
                ...
                (Γηπεδούχος)|       |(Φιλοξενούμενη)
                            |       |
                      < Διεξάγεται >  <-- Ένας ΑΓΩΝΑΣ συνδέει
                        /         \        δύο ρόλους της ΟΜΑΔΑΣ
                       v           v
                  +-----------------------+
                  |        ΑΓΩΝΑΣ         |
                  +-----------------------+
...
**Αντιστοίχιση στο σχεσιακό μοντέλο**: Κάθε συσχέτιση-ρόλος μετατρέπεται σε **ξεχωριστό ξένο κλειδί** στον ίδιο πίνακα.

```sql
-- Δύο ξένα κλειδιά στον ίδιο πίνακα για τους δύο ρόλους
...
**Exam Note:** Όταν το ίδιο ζεύγος οντοτήτων εμφανίζεται με δύο διαφορετικές σημασίες, **μην** το συγχέετε με την αναδρομική (unary) συσχέτιση. Εδώ πρόκειται για **δύο διακριτές συσχετίσεις** με ρόλους, καθεμία από τις οποίες αποτυπώνεται ως ξεχωριστό ξένο κλειδί.

---

## Εξειδίκευση / Γενίκευση (ISA)
*Specialization / Generalization (ISA)*

Η **Εξειδίκευση (Specialization)** και η **Γενίκευση (Generalization)** αποτελούν μηχανισμούς αφαίρεσης του E-R μοντέλου για τη μοντελοποίηση ιεραρχιών τύπων (subtyping), όπου μια οντότητα-γονέας διακρίνεται σε υποκατηγορίες.

- **Γενίκευση (Generalization)**: Αφαίρεση προς τα πάνω — κοινά γνωρίσματα επιμέρους οντοτήτων ενοποιούνται σε έναν **υπερτύπο (supertype)**.
- **Εξειδίκευση (Specialization)**: Αφαίρεση προς τα κάτω — ένας υπερτύπος διαιρείται σε **υποτύπους (subtypes)** με πρόσθετα, εξειδικευμένα γνωρίσματα.
- Η σχέση κληρονομικότητας συμβολίζεται με τη λέξη **ISA** ("is a") — κάθε υποτύπος **είναι** ένας υπερτύπος.

**Παράδειγμα**: Ο υπερτύπος `ΤΙΤΛΟΣ_ΜΕΣΩΝ` (media titles) εξειδικεύεται σε `ΤΑΙΝΙΑ` (με γνώρισμα `διάρκεια`) και `ΣΕΙΡΑ` (με γνώρισμα `αριθμός_επεισοδίων`).

```text
                     +------------------+
                     |   ΤΙΤΛΟΣ_ΜΕΣΩΝ   |   <-- Υπερτύπος
                     +------------------+
                      /              \
                     ISA              ISA
                    /                  \
          +---------+                +---------+
          |  ΤΑΙΝΙΑ  |                |  ΣΕΙΡΑ  |   <-- Υποτύποι
          +---------+                +---------+
          | διάρκεια|                | επεισόδια|
          +---------+                +---------+
```
...
- **Ολική (total) vs Μερική (partial) εξειδίκευση**: Ολική σημαίνει ότι κάθε στιγμιότυπο του υπερτύπου **πρέπει** να ανήκει σε κάποιον υποτύπο. Μερική σημαίνει ότι μπορεί να υπάρχουν στιγμιότυπα μόνο του υπερτύπου.
- **Ξένη (disjoint) vs Επικαλυπτόμενη (overlapping) εξειδίκευση**: Ξένη σημαίνει ότι ένα στιγμιότυπο ανήκει **το πολύ σε έναν** υποτύπο. Επικαλυπτόμενη σημαίνει ότι μπορεί να ανήκει **σε πολλούς** υποτύπους ταυτόχρονα.

**Key Distinction:** Σε ξένη (disjoint) εξειδίκευση οι υποτύποι είναι αμοιβαία αποκλειόμενοι (ένα άτομο είναι είτε φοιτητής είτε καθηγητής, όχι και τα δύο). Σε επικαλυπτόμενη, ένα άτομο μπορεί να είναι ταυτόχρονα και φοιτητής και υπάλληλος.
...
| **Περιορισμός Συμμετοχής** | Αν η συμμετοχή οντότητας είναι υποχρεωτική ή προαιρετική | Ολική (διπλή γραμμή) ή Μερική (μονή γραμμή) |
| **Ολική Συμμετοχή** | Κάθε στιγμιότυπο πρέπει να συμμετέχει στη συσχέτιση | FK `NOT NULL` στο σχεσιακό μοντέλο |
| **Μερική Συμμετοχή** | Κάποια στιγμιότυπα μπορεί να μη συμμετέχουν | FK επιτρέπει `NULL` |
| **Γνώρισμα Συσχέτισης** | Ιδιότητα που ανήκει στη συσχέτιση, όχι στις οντότητες | Σε N:M πάει στον ενδιάμεσο πίνακα |
| **Πολλαπλές Συσχετίσεις** | Δύο διαφορετικές σχέσεις μεταξύ ίδιων οντοτήτων | Διακρίνονται με ρόλους — χωριστό FK ανά ρόλο |
| **Εξειδίκευση / Γενίκευση (ISA)** | Ιεραρχία υπερτύπου-υποτύπων | Περιορισμοί: ολική/μερική, ξένη/επικαλυπτόμενη |
...
- **Key Distinction:** Η **πληθικότητα** απαντά "πόσα" στιγμιότυπα συνδέονται, ενώ ο **περιορισμός συμμετοχής** απαντά "αν είναι υποχρεωτικό". Η **ολική συμμετοχή** (διπλή γραμμή) κάνει το αντίστοιχο ξένο κλειδί `NOT NULL`, η **μερική** (μονή γραμμή) το επιτρέπει `NULL`.
- Τα **γνωρίσματα συσχέτισης** ανήκουν στη συσχέτιση και όχι στις οντότητες — σε N:M συσχέτιση καταλήγουν στον ενδιάμεσο πίνακα, μαζί με τα δύο ξένα κλειδιά.
- Δύο διαφορετικές **συσχετίσεις μεταξύ ίδιων οντοτήτων** διακρίνονται με ετικέτες ρόλων και αντιστοιχούν σε ξεχωριστά ξένα κλειδιά (π.χ. ομάδα γηπεδούχος και φιλοξενούμενη σε έναν αγώνα).
- Η **Εξειδίκευση/Γενίκευση (ISA)** μοντελοποιεί ιεραρχίες υπερτύπου-υποτύπων με περιορισμούς ολική/μερική και ξένη/επικαλυπτόμενη — χρήσιμη όταν ένας γενικός τύπος διακρίνεται σε ειδικότερες κατηγορίες.
