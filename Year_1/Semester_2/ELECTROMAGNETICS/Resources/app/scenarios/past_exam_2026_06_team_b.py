    ...
    questions = [
        ...
        ExamQuestion(
            ...
            calculation_steps=[
                CalculationStep(
                    ...
                    rationale="Η απόκλιση της ηλεκτρικής μετατόπισης ισούται ακριβώς με την τοπική χωρική πυκνότητα ελεύθερων ηλεκτρικών φορτίων.",
                ),
                CalculationStep(
                    step_number=2,
                    title="Αναγνώριση και καταγραφή των συνιστωσών του διανύσματος D",
                    formula=r"\vec{D} = D_x \hat{x} + D_y \hat{y} + D_z \hat{z}",
                    substitution=r"D_x = x^2, \quad D_y = 0, \quad D_z = -zy",
                    result=r"(D_x, D_y, D_z) = (x^2, \ 0, \ -zy)",
                    rationale="Αντιστοίχιση των διανυσματικών συνιστωσών της εκφώνησης στους άξονες x, y και z.",
                ),
                CalculationStep(
                    step_number=3,
                    title="Υπολογισμός μερικής παραγώγου της x-συνιστώσας",
                    formula=r"\frac{\partial D_x}{\partial x} = \frac{\partial}{\partial x}(x^2)",
                    substitution=r"\frac{\partial}{\partial x}(x^2) = 2 x^{2-1} = 2x",
                    result=r"\frac{\partial D_x}{\partial x} = 2x",
                    rationale="Βασικός κανόνας παραγώγισης απλής δύναμης d(x^2)/dx = 2x.",
                ),
                CalculationStep(
                    step_number=4,
                    ...
                    rationale="Η συνιστώσα του πεδίου στον άξονα y είναι μηδενική, συνεπώς και η παράγωγός της ισούται με 0.",
                ),
                CalculationStep(
                    step_number=5,
                    title="Υπολογισμός μερικής παραγώγου της z-συνιστώσας",
                    formula=r"\frac{\partial D_z}{\partial z} = \frac{\partial}{\partial z}(-zy) = -y \cdot \frac{\partial}{\partial z}(z)",
                    substitution=r"\frac{\partial D_z}{\partial z} = -y \cdot (1) = -y",
                    result=r"\frac{\partial D_z}{\partial z} = -y",
                    rationale="Κατά τη μερική παραγώγιση ως προς z, η μεταβλητή y αντιμετωπίζεται ως σταθερός πολλαπλασιαστικός παράγοντας. Προσοχή στο αρνητικό πρόσημο.",
                ),
                CalculationStep(
                    step_number=6,
                    ...
                    substitution=r"\rho = 2x + 0 + (-y) = 2x - y",
                    result=r"\rho(x, y, z) = 2x - y \quad [\text{C/m}^3]",
                    rationale="Η τοπική πυκνότητα φορτίου μεταβάλλεται γραμμικά με τις συντεταγμένες θέσης (x, y) και είναι ανεξάρτητη του z.",
                ),
            ],
        ...
        ExamQuestion(
            ...
            calculation_steps=[
                CalculationStep(
                    step_number=1,
                    title="Αναγνώριση βασικών κυματικών παραμέτρων από την εξίσωση του B",
                    formula=r"\vec{B}(z,t) = B_0 \cos(kz - \omega t)\hat{u}_B",
                    substitution=r"B_0 = 3 \ \text{T}, \quad k = 100\pi \ \text{rad/m}, \quad \hat{u}_B = \hat{y}, \quad \hat{k} = +\hat{z}",
                    result=r"B_0 = 3 \ \text{T}, \quad k = 100\pi \ \text{rad/m}",
                    rationale="Η έκφραση περιγράφει αρμονικό κύμα διαδιδόμενο προς τα +z με μαγνητικό πεδίο πολωμένο κατά y.",
                ),
                CalculationStep(
                    step_number=2,
                    title="Αναλυτικός υπολογισμός μήκους κύματος λ",
                    formula=r"\lambda = \frac{2\pi}{k}",
                    substitution=r"\lambda = \frac{2\pi}{100\pi} = \frac{2}{100} \ \text{m} = 0.02 \ \text{m} = 2 \ \text{cm}",
                    result=r"\lambda = 0.02 \ \text{m} = 2 \ \text{cm}",
                    rationale="Το π απλοποιείται απευθείας δίνοντας ακριβές μήκος κύματος 2 εκατοστά.",
                ),
                CalculationStep(
                    step_number=3,
                    title="Αναλυτικός υπολογισμός κυκλικής συχνότητας ω",
                    formula=r"\omega = c \cdot k",
                    substitution=r"\omega = (3 \times 10^8 \ \text{m/s}) \times (100\pi \ \text{rad/m}) = 3 \times 10^{10}\pi \ \text{rad/s}",
                    result=r"\omega = 3 \times 10^{10}\pi \ \text{rad/s} \approx 9.4248 \times 10^{10} \ \text{rad/s}",
                    rationale="Γραμμική σχέση διασποράς στο κενό χωρίς υλικές απώλειες.",
                ),
                CalculationStep(
                    step_number=4,
                    title="Υπολογισμός γραμμικής συχνότητας f",
                    formula=r"f = \frac{\omega}{2\pi} = \frac{c}{\lambda}",
                    substitution=r"f = \frac{3 \times 10^{10}\pi}{2\pi} = 1.5 \times 10^{10} \ \text{Hz} = 15 \ \text{GHz}",
                    result=r"f = 15 \ \text{GHz} = 1.5 \times 10^{10} \ \text{Hz}",
                    rationale="Η συχνότητα 15 GHz ανήκει στη ζώνη μικροκυμάτων Ku (δορυφορικές επικοινωνίες).",
                ),
                CalculationStep(
                    step_number=5,
                    title="Υπολογισμός πλάτους ηλεκτρικού πεδίου E_0",
                    formula=r"E_0 = c \cdot B_0",
                    substitution=r"E_0 = (3 \times 10^8 \ \text{m/s}) \times (3 \ \text{T}) = 9 \times 10^8 \ \text{V/m}",
                    result=r"E_0 = 9 \times 10^8 \ \text{V/m}",
                    rationale="Επειδή δίνεται το B_0, πολλαπλασιάζουμε με το c για να βρούμε το πλάτος E_0.",
                ),
                CalculationStep(
                    step_number=6,
                    title="Προσδιορισμός διεύθυνσης ηλεκτρικού πεδίου E μέσω εξωτερικού γινομένου",
                    formula=r"\hat{k} = \hat{E} \times \hat{B} \implies \hat{z} = \hat{E} \times \hat{y} \implies \hat{E} = +\hat{x}",
                    substitution=r"\vec{E}(z,t) = E_0 \cos(kz - \omega t)\hat{x} = 9 \times 10^8 \cos(100\pi z - 3\times 10^{10}\pi t)\hat{x} \ \text{V/m}",
                    result=r"\vec{E}(z,t) = 9 \times 10^8 \cos(100\pi z - 3\times 10^{10}\pi t)\hat{x} \ \text{V/m}",
                    rationale="Επειδή x̂ × ŷ = +ẑ, το ηλεκτρικό πεδίο πρέπει να ταλαντώνεται αυστηρά κατά +x̂.",
                ),
                CalculationStep(
                    step_number=7,
                    ...
                    substitution=r"\vec{S} = \frac{(9 \times 10^8)(3)}{4\pi \times 10^{-7}}\cos^2(...)(+\hat{z}) = \frac{2.7 \times 10^{16}}{4\pi}\cos^2(100\pi z - \omega t)\hat{z}",
                    result=r"\vec{S}(z,t) \approx 2.1486 \times 10^{15} \cos^2(100\pi z - 3\times 10^{10}\pi t)\hat{z} \ \text{W/m}^2",
                    rationale="Η ροή ενέργειας ανά μονάδα επιφάνειας κατευθύνεται κατά μήκος της διάδοσης +ẑ.",
                ),
                CalculationStep(
                    step_number=8,
                    title="Υπολογισμός μέσης έντασης ακτινοβολίας I",
                    formula=r"I = \langle |\vec{S}| \rangle = \frac{S_{\text{peak}}}{2} = \frac{E_0 B_0}{2\mu_0}",
                    substitution=r"I = \frac{2.1486 \times 10^{15}}{2} \approx 1.0743 \times 10^{15} \ \text{W/m}^2",
                    result=r"I \approx 1.0743 \times 10^{15} \ \text{W/m}^2",
                    rationale="Η χρονική μέση τιμή του cos² σε μία περίοδο είναι 1/2.",
                ),
            ],
