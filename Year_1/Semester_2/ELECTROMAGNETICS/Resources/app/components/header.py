"""Header navigation component for the Electromagnetics application."""
...
def renderHeader(
    ...
    initial_subtitle = (
        ...
        if current_scenario
        else "Πλήρης Θεωρία, Εξισώσεις Maxwell & Κυματική Διάδοση"
    )
    initial_course_tag = (
    ...
    with ui.header().classes(
        ...
        with ui.row().classes("items-center gap-4"):
            ui.html('<i class="fa-solid fa-bolt text-[var(--accent)] text-2xl"></i>')
            with ui.column().classes("gap-0"):
                ui.html('<h1 class="gradient-title text-xl md:text-2xl font-black">Αρχές Ηλεκτρομαγνητισμού & Τηλεπικοινωνιών</h1>')
