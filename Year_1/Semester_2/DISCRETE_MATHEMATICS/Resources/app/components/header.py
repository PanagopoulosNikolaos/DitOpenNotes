"""Header navigation component for Course 203: Discrete Mathematics application."""
...
def renderHeader(
    ...
    """Renders the top application header with title, scenario selector, theme switch, and print menu.
    ...
    initial_subtitle = (
        ...
        if current_scenario
        else "Πλήρης Θεωρία, Μεθοδολογία & Αλγοριθμικό Εργαστήριο"
    )
    initial_course_tag = (
    ...
    with ui.header().classes(
        ...
        with ui.row().classes("items-center gap-4"):
            ...
            with ui.column().classes("gap-0"):
                ui.html('<h1 class="gradient-title text-xl md:text-2xl font-black">Διακριτά Μαθηματικά</h1>')
            ...
            with ui.button("Εκτύπωση / PDF", icon="print").props("outline dense").classes(
                ...
                with ui.menu().classes("bg-[var(--menu-bg)] text-[var(--text-1)] border border-[var(--border-accent)] shadow-xl"):
                    ui.menu_item(
                        ...
                        on_click=lambda: ui.run_javascript("printDiscreteSection('all');"),
                    ).classes("text-xs hover:bg-[var(--surface-hover)] font-bold text-[var(--accent)]")
                    ui.separator().classes("bg-[var(--border)]")
                    ui.menu_item(
                        "1. Θέματα Εξέτασης & Λύσεις",
                        on_click=lambda: ui.run_javascript("printDiscreteSection('problems');"),
                    ).classes("text-xs hover:bg-[var(--surface-hover)]")
                    ui.menu_item(
                        "2. Διαδραστικά Εργαστήρια & Γράφοι",
                        on_click=lambda: ui.run_javascript("printDiscreteSection('simulator');"),
                    ).classes("text-xs hover:bg-[var(--surface-hover)]")
                    ui.menu_item(
                        "3. Οδηγός Θεωρίας & Ύλης",
